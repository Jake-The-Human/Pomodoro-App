...
def getCommandLineArgs():
	...
	parser.add_argument(
		'-nw',
		action="store_true",
		help='This flag will make the script run with out the GUI so it will run in the terminal'
	)
	parser.add_argument(
		'Focus timer length', 
		metavar='F_Time',
		type=int,
		nargs='?',
		help='The number of minutes you want to focus for. Default: '+str(ConstValues.WORK_MINUTES)+' minutes'
	)
	parser.add_argument(
		'Break timer length',
		metavar='B_Time',
		type=int,
		nargs='?',
		help='The number of minutes you want to break for. Default: '+str(ConstValues.BREAK_MINUTES)+' minutes'
	)
	# parser.add_argument(
